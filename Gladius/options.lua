...
function Gladius:SetupOptions()
				...
				args = {
					...
					trinket = {
						...
						args = {
							...
							bigTrinketOffsetX = {
								type="range",
								name=L["Big icon OffsetX"],
								desc=L["OffsetX for the big trinket icon"],
								min=-10,
								max=10,
								step=.01,
								disabled = function() return not self.db.profile.trinketStatus or self.db.profile.trinketDisplay ~= "bigIcon" and self.db.profile.trinketDisplay ~= "bigGridIcon" end,
							...
							bigTrinketOffsetY= {
								type="range",
								name=L["Big icon OffsetY"],
								desc=L["OffsetY for the big trinket icon"],
								min=-10,
								max=10,
								step=.01,
								disabled = function() return not self.db.profile.trinketStatus or self.db.profile.trinketDisplay ~= "bigIcon" and self.db.profile.trinketDisplay ~= "bigGridIcon" end,
	...
	for class, spells in pairs(self.cooldownSpells) do
      self.options.args.cooldowns.args[class] = {
         type = "group",
         order = 1,
         name = LOCALIZED_CLASS_NAMES_MALE[class] or class,
         args = {
            cooldownToggle = {
               type="toggle",
